...
function valid_save_data() {
    ...
    if (getRow) {
        if (main_form.score.value > getRow.allocation) {
            alert_msg = "計分大於指標配分，不允存檔\n";
            rtn_val = false;
        }
    }
    ...
    if (parseInt(main_form.lower_limit.value) >= parseInt(main_form.upper_limit.value)) {
...
config.employee_info_easy_dg = {
    ...
    onSelect: function (index, row) {
        ...
        if (parseInt(searchYear)>2020){
            eastSourceUrl = "/api/get_metrics_setupDate_data_search/" + work_code_key + "/" + searchYear;
        } else {
...
config.metrics_setupDate_dg = {
    ...
    onLoadSuccess: function (data) {
        //如果當年度沒資料, 就清空grid
        find_month = parseInt(workingMonth);
...
config.metrics_setup_dg = {
    ...
    columns: [[
        ...
        {
            field: 'metrics_content', title: '衡量指標(一定要有數字)', width: 1000,
...
config.main_dg = {
    ...
    columns: [[
        ...
        {field: 'lower_limit', title: '下限&nbsp;&nbsp;<br>( >= )', width: 100, align: 'right',},
        {field: 'upper_limit', title: '上限<br>( < )', width: 100, align: 'right',},
        {
...
　　centerControl('update');
    ...
    $('#south_search_open').click(function () {
        ...
        $('#south_search_sumit').click(function () {
            ...
            southSourceUrl = "/api/get_metrics_setup_data/" + searchWorkCode + "/" + searchChiName + "/" + searchDept + "/all";
            // $('#employee_info_easy_dg').datagrid({autoLoad:true});
            // $('#employee_info_easy_dg').datagrid('reload');
