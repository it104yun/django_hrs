...
function exportData_singleSheet(from_bpm) {
   var job_title_desc = "";
   var tab_dg = "";
   ...
   for (var index = 0; index < tab_numbers; index++) {
       ...
       var fileName = main_form.id_work_code.value+main_form.id_chi_name.value+"-"+job_title_desc+"-PDCA-"+main_form.id_the_time.value.padStart(2,"0")+"version-"+getFormatedDateTime();
       if ( from_bpm == "Y"){
           fileName = main_form.id_work_code.value+main_form.id_chi_name.value+"-"+job_title_desc+"-PDCA-"+main_form.id_the_time.value.padStart(2,"0")+"version-sign-backup-"+getFormatedDateTime();
       }
...
function config_tabs_datagrid(add_version){
    ...
    for (var i=0 ; i<len ; i++){
        tab_dg = '#tab'+tabs[i].job_title+'_dg'
        tab_url="/sk_api/pdca_detail/"+tabs[i].job_title+"/"+userId+"/"+add_version;
        $(tab_dg).datagrid( {
            ...
            onDblClickRow: function (index,row) {
                ...
                if (( last_work_item == "" || last_work_item == null || last_work_item == "　") && index!=0){
                        ...
                        alert(alert_str);
                        // alert("NO : " + (lastRowIndex + 1).toString() + "　以下資料未填寫完成,不能選其他列,儲存也不會成功\n\n1-工作項目 \n2- Ｐ、Ｄ、Ｃ、Ａ至少填一個　\n3- 週期、次數都必須填寫");
...
$(document).ready(function(){
    ...
    if ( pdca_agent=="True" ) {
        alert(gettext("工作事項明細表,您無法自行填寫,若有任何疑問,請洽詢『人資中心』!"));
        $('#main_form').hide();
        $('#main-tab').hide();
        $('#form_choice_lang').hide();
    } else {
        var tab_numbers = tabs.length;
        var tab_dg;
        var rowsNumber;
        ...
        //2022/06/20-------add------------begin
        // 更新select_work_code...好讓"共用的"--"submit_bpm_pdca"-->api....可以正常更新"bpm送簽狀態"
        $.ajax({
            type: "get",
            url: '/sk_api/update_session_pdca/' + main_form.id_work_code.value + "/" + main_form.id_work_code.chi_name,
        ...
        function tab_save(bpm_yn) {
            ...
            for (var i = 0; i < tab_numbers; i++) {
                ...
                redefine_dg_footer($(tab_dg));
                mergeCells_dg_footer($(tab_dg));
                tabData = get_datagrid_rows($(tab_dg), tabs[i].job_title, tabs[i].job_title_desc, $('#id_work_code').val(), $('#id_the_time').val(), bpm_yn);      //取得每個TAB的GridRows，
                tabFooterData = get_footer_rows($(tab_dg), tabs[i].job_title, tabs[i].job_title_desc, $('#id_work_code').val(), $('#id_the_time').val());
        ...
        $("#preview_report").click(function () {
            ...
            var URLToPDF = location.href + '_2pdf?fileName=' + fileName;
            window.open(URLToPDF);  // 這裏只是去執行 class tt602_2pdf 啟動這個view, 讓他產生html, 好可以轉換成pdf, 開啟此pdf的template為tt602_2pdf_open.html
        })
        ...
        $("#submit_bpm").click(function () {
            ...
            for (var i = 0; i < tab_numbers; i++) {
                ...
                if (tabNotEmptyCount == 0) {
                    tabEmpty.push(tabs[i].job_title_desc)
                }
            }
            ...
            if (err_len > 0 || empty_len > 0) {     //有錯誤訊息 or 有tab為空
                ...
                window.open(location.href + '_2pdf?fileName=' + fileName);
            }
        })
    }
});
