...
function editrow(target,tab_dg){
    // $(tab_dg).datagrid('beginEdit', getRowIndex(target));
    $(tab_dg).datagrid('beginEdit', 0);
}

function deleterow(target,tab_dg){
    $.messager.confirm('Confirm','Are you sure?',function(r){
        if (r){
            $(tab_dg).datagrid('deleteRow', getRowIndex(target));
        }
    });
}

function saverow(target,tab_dg){
    $(tab_dg).datagrid('endEdit', getRowIndex(target));
}

function cancelrow(target,tab_dg){
    $(tab_dg).datagrid('cancelEdit', getRowIndex(target));
}
...
function reConfigGridColumns(i) {
                    ...
                    editor: { type:'combobox',
                                options:{
                                        ...
                                        onChange: function (newValue,oldValue) {
                                            ...
                                            var index = $('#main-tab').tabs('getTabIndex',sel_tab);
                                            var tab_dg = '#tab' + tabs[index].job_title + '_dg';
                                            var sel_row = $(tab_dg).datagrid('getSelected');
                                            if ( sel_row == null){
                                                sel_row = $(tab_dg).datagrid('selectRow',0)
                                            }
                                            var sel_work_code = sel_row.work_code;
                                            var ee = ee_jobs.find( function (ee){   return ee.work_code == sel_work_code  } );   //找出每個人有多少職務
                                            var jobs = ee.job_name;
                                            var job_len = jobs.length;
    ...
    /*
    if ( tab_len > 8 && jj==0 ) {
        arr_title_group.push({title: 'Action', colspan: 0, align: 'center',
            styler: function (value, row, index) {
                        return 'color:Yellow;background-color: Black;font-size:12px;';
                    },
        });
        arr_title.push({
            field:'action',
                formatter:function(value,row,index){
                    if (row.editing){
                        var s = '<a href="javascript:void(0)" id="save_row" ondblclick="saverow(this,\'#tab\' + tabs[0].job_title + \'_dg\')">Save</a> ';
                        var c = '<a href="javascript:void(0)" id="cancel_row" ondblclick="cancelrow(this,\'#tab\' + tabs[0].job_title + \'_dg\')">Cancel</a>';
                        return s+c;
                    } else {
                        var e = '<a href="javascript:void(0)" id="edit_row" ondblclick="editrow(this,\'#tab\' + tabs[0].job_title + \'_dg\')">Edit</a> ';
                        var d = '<a href="javascript:void(0)" id="delete_row" ondblclick="deleterow(this,\'#tab\' + tabs[0].job_title + \'_dg\')">Delete</a>';
                        return e+d;
                    }
            }
        })
    ...
    for (var i=0 ; i<len ; i++){
        ...
        $(tab_dg).datagrid( {
            ...
            onLoadSuccess: function(data) {
                // 2022/04/07....add
                //只有在全部載入,才會執行onLoadSuccess,只有顯示最後一個tab_dg 及 i=10
                if ( len > 8) {                                          //2022/04/22加入
                    var editorRow = $(this).datagrid('getRows')[0];
                    $(this).datagrid('selectRow', 0);   //相當於第1次click
                    $(this).datagrid('beginEdit', 0);
                    rowEditorControl($(this), 0, editorRow);       //2022/04/22加入
                }
            }
        });
